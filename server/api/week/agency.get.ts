...
export default defineEventHandler(async (event) => {
  ...
  const allBookingsRaw = timeslotIds.length > 0
    ? await db.select().from(schema.bookings).where(
        inArray(schema.bookings.timeslotId, timeslotIds),
      )
    : [];
  const allBookings = allBookingsRaw;
  const agencyBookings = allBookingsRaw.filter(b => b.agencyId === auth.userId);

  const [users, guestLists, guideAssignments] = await Promise.all([
  ...
  const agencyBookingsBySlot = new Map();
  agencyBookings.forEach((booking: any) => {
    ...
    if (ts) {
      ...
      if (day) {
        const timeStr = typeof ts.time === "string" ? ts.time.substring(0, 5) : `${String(ts.time.hours || 0).padStart(2, "0")}:${String(ts.time.minutes || 0).padStart(2, "0")}`;
        const key = `${day.date}|${timeStr}`;
        if (!agencyBookingsBySlot.has(key)) {
          agencyBookingsBySlot.set(key, []);
        }
        agencyBookingsBySlot.get(key)!.push({ ...booking, date: day.date, time: timeStr });
      }
    }
  });

  const allBookingsBySlot = new Map();
  allBookings.forEach((booking: any) => {
    ...
    if (ts) {
      ...
      if (day) {
        const timeStr = typeof ts.time === "string" ? ts.time.substring(0, 5) : `${String(ts.time.hours || 0).padStart(2, "0")}:${String(ts.time.minutes || 0).padStart(2, "0")}`;
        const key = `${day.date}|${timeStr}`;
        if (!allBookingsBySlot.has(key)) {
          allBookingsBySlot.set(key, []);
        }
        allBookingsBySlot.get(key)!.push({ ...booking, date: day.date, time: timeStr });
      }
    }
  });
  ...
  const result = weekDates.map((dateStr) => {
    ...
    const timeslots = allTimes.map((timeStr) => {
      ...
      const myBookings = agencyBookingsBySlot.get(key) || [];
      const allSlotBookings = allBookingsBySlot.get(key) || [];

      const limit = slot?.limit ?? dayLimit;
      const booked = allSlotBookings
